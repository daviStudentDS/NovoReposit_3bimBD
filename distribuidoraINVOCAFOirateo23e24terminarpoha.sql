...
-- call spInsertCompra(2482, 'Revenda Chico Loco', '2020-04-22', 12345678910118, 40.50, 180, 180, 7290.00);
...
call spInsertVenda(4, 'Disney Chaplin', 12345678910247, 64.50, 1, 64.50, null);
call spInsertVenda(4, 'Lança Perfume', 12345678910112, 64.50, 1, 64.50, null);
...
delimiter $$
create procedure spInsertVenda2(vNumVenda int, vNome varchar(50), vDataVenda date, vCodigoBarras bigint, vQtd int)
begin
	if exists (select * from tbProduto, tbCliente where CodBarras = vCodigoBarras and Nome = vCliente) then
		if not exists(select * from tbVenda where NumeroVenda = vNumVenda) then
			set @dataVenda = str_to_date(vDataVenda, '%d-%m-%Y');
            set @valorItem = (select ValorItem from tbProduto where CodBarras = vCodigoBarras);
			set @id = (select Id from tbCliente where Nome = vCliente);
			insert into tbVenda(NumeroVenda,Id,DataVenda,ValorTotal, QtdTotal) values (vNumVenda,@idCliente,@dataVenda,(vQtd * @valorItem),vQtd);

		end if;
      if not exists (select * from tbItemVenda where NumeroVenda = vNumVenda) then
			insert into tbItemVenda(NumeroVenda,CodBarras,ValorItem,Qtd) values (vNumVenda, vCodigoBarras,@valorItem,vQtd);
        end if;
    end if;
   
	if not exists(select * from tbCliente where Nome = vCliente) then select("Registro do produto não existe"); end if;
	if not exists(select * from tbProduto where CodBarras = vCodigoBarras) then select("Registro do cliente não existe"); end if;
end
$$

call spInsertVenda2(4, "Disney Chaplin","2022-09-26",12345678910111,1);
